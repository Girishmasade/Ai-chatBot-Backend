...
// get admin profile
export const getAdminProfile = AsyncHandler(async (req: Request, res: Response, next: NextFunction) => {
...
export const getUsers = AsyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  ...
  const subMap = new Map(subscriptions.map((s: any) => [s.user?.toString() || "", s.planName || "free"]));
  ...
  const mappedUsers = users.map((u) => ({
    ...
    status: (u as any).status || ((u as any).isBlocked ? "blocked" : "active"),
  }));

  successHandler(res, 200, true, "Users fetched dynamically", { data: mappedUsers });
});
...
export const deleteUser = AsyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  ...
  successHandler(res, 200, true, "User status disabled successfully", { data: user });
});

// get models
export const getModels = AsyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  const CACHE_KEY = "cache:admin:models";
  try {
    const cached = await redisClient.get(CACHE_KEY);
    if (cached) {
      return successHandler(res, 200, true, "Models fetched (from Redis cache)", { data: JSON.parse(cached) });
    }
  } catch (err) {
...
// toggle model status
export const toggleModel = AsyncHandler(async (req: Request, res: Response, next: NextFunction) => {
...
export const getSubscriptions = AsyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  ...
  successHandler(res, 200, true, "Subscriptions fetched", { data: mappedSubs });
});
...
export const getLogs = AsyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  ...
  const mappedLogs = logs.map((l) => ({
    ...
    action: l.action,
    operator: l.operator,
    timestamp: l.createdAt ? new Date(l.createdAt).toLocaleString() : new Date().toLocaleString(),
    details: l.details,
    level: l.level,
  }));

  successHandler(res, 200, true, "Audit logs fetched from database", { data: mappedLogs });
});
...
export const getConfig = AsyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    const cached = await redisClient.get(CACHE_KEY);
    if (cached) {
      return successHandler(res, 200, true, "Config fetched (from Redis cache)", { data: JSON.parse(cached) });
    }
  } catch (err) {
    console.error("Redis read error:", err);
  }
  ...
  const mappedConsents = cookieConsents.map((c) => ({
    ...
    timestamp: (c as any).createdAt ? new Date((c as any).createdAt).toLocaleString() : new Date().toLocaleString(),
  }));
...
export const updateBranding = AsyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  ...
  emitAdminEntityUpdate({ entityType: "config", action: "updated", data: branding });

  successHandler(res, 200, true, "Branding updated in database", { data: branding });
});

// log cookie consent
export const logCookieConsent = AsyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  ...
  successHandler(res, 201, true, "Cookie consent recorded", { data: consent });
});
...
export const getUserAssets = AsyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  ...
  const assets = await AIAssetModel.find({ user: userId }).sort({ createdAt: -1 }).lean();
  ...
  const mappedAssets = assets.map((a) => ({
    ...
    content: a.content,
    model: a.model,
    timestamp: (a as any).createdAt ? new Date((a as any).createdAt).toLocaleString() : new Date().toLocaleString(),
  }));
